...
void split(std::string name, std::vector<std::string> &names_upper,
           std::vector<std::string> &names_lower) {
  std::string delimiter{", "};
  std::size_t pos{0};
  while ((pos = name.find(delimiter)) != std::string::npos) {
    std::string token = name.substr(0, pos);
    is_all_upper(token) ? names_upper.push_back(token)
                        : names_lower.push_back(token);
    name.erase(0, pos + delimiter.length());
  }
  is_all_upper(name) ? names_upper.push_back(name)
                     : names_lower.push_back(name);
}
...
std::string greet_toupper(std::string greeting) {
  std::transform(std::begin(greeting), std::end(greeting), std::begin(greeting),
                 ::toupper);
...
const std::string &greet(const std::string &name) {
  ...
  std::vector<std::string> names_lower;
  if (name.length() == 0)
    names_lower.push_back("my friend");
  else
    split(name, names_upper, names_lower);
...
TEST(greetSuite, Empty) { EXPECT_EQ("Hello, my friend.", greet("")); }

TEST(greetSuite, UpperCase) { EXPECT_EQ("HELLO, BOB!", greet("BOB")); }

TEST(greetSuite, OneName) { EXPECT_EQ("Hello, Bob.", greet("Bob")); }
...
TEST(greetSuite, MoreNames) {
  ...
  EXPECT_EQ("Hello, Adam, Bob, Charlie and Dan.",
            greet("Adam, Bob, Charlie, Dan"));
}

TEST(greetSuite, MixedNamed) {
  EXPECT_EQ("Hello, Adam and Charlie. AND HELLO BOB!",
            greet("Adam, BOB, Charlie"));
}
